...
def text_emotion(MyText):
    test=pd.read_csv('Scripts//x_test.csv').squeeze()
    test.loc[len(test)] = MyText
    text_model=joblib.load('Scripts//Naive_Bayes.sav')
    print(test.iloc[-1])
...
def getemotion(audio,MyText):
    ...
    for key in speech_emotion_preds:
        for key in text_emotion_preds:
            final_dic[key] =((speech_emotion_preds[key]*0.4) + (text_emotion_preds[key]*0.6))/2
