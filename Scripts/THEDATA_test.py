...
def extract_feature(filename):
    X, sample_rate = librosa.load(filename,duration=3,sr=44100,offset=0.5)
    mfcc=np.mean(librosa.feature.mfcc(y=X,sr=sample_rate,n_mfcc=40).T,axis=0)
#     print(0)
    return mfcc

def getemotion(audio):
    ...
    if Result==["male_neutral"] or Result==["female_neutral"]:
        emotion='neutral'
    if Result==["male_angry"] or Result==["female_angry"] :
        emotion='angry'
    if Result==["male_fear"] or Result==["female_fear"]:
        emotion='fear'
    if Result==["male_sad"] or Result==["female_sad"]:
        emotion='sad'
    if Result==["male_disgust"] or Result==["female_disgust"]:
        emotion='neutral'
    if Result==["male_happy"] or Result==["female_happy"]:
        emotion='happy'
    print(emotion)
    return emotion
...
def text_emotion(text):
    textmodel=joblib.load('RF.sav')
    pred= predict(textmodel, text, type="prob")
    print(pred)
    return pred

def average_models(probs1,probs2):
    ...
    # 8: 'Normal'}    
    new_prob = []
    for val1,val2 in zip(probs1,probs2):
        val = (val1+val2)/2
        new_prob.append(val)
    prob = max(new_prob)
    pred_idx = new_prob.index(prob)
...
def combined(emotion,text_emotion):
    return 0
def generater(tokenizer, START_TOKEN,END_TOKEN,VOCAB_SIZE,audiofile):
    ...
    try:
                ...
                #GETTING EMOTIION
                audio=audiofile
                emotion=getemotion(audio)
                # text_emotion=text_emotion(MyText)

                # combined_emo=combined(emotion,text_emotion)
                #GETTING REPLY
                optimizer=optimize()
                test=create_model(optimizer)
                test.load_weights('Scripts\model_with_THEDATA_new.h5')
